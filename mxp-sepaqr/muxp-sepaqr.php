...
define('muxp_BACS_COMPANY','Zahlungsempfaenger'); // optional: insert your business name as fallback
define('muxp_PURPOSE_PREFIX','Order-ID / Bestell-Nr.:'); // this string will be prefixed to  the order ID in SEPA purpose Text
...
function muxp_qrpage_from_hash($template) {
	...
	if ( false === muxp_valid_md5($md5) ) { 
		$html = '<p>This is not a valid md5 hash :-(<br>Generating uncached dummy QR for 11 Euro/OrderID 11<br><img src="' . muxp_create_qrcode(11,11) . '"></p>';
...
function muxp_create_qrcode ($amount,$orderid) {
 ...
 $payloadtext = muxp_PURPOSE_PREFIX . ' ' . $orderid;
